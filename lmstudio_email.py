...
communications. It uses LM Studio (local model server) for email classification and hotel name
extraction, and the Google Places API for fetching additional hotel details.
...
Dependencies:
- LM Studio running locally (e.g., at http://localhost:1234)
...
# LM Studio setup
LM_STUDIO_URL = config.LM_STUDIO_API_URL
HEADERS = {"Content-Type": "application/json"}

# Database setup
Base = declarative_base()
engine = create_engine('sqlite:///lmstudio_email_processing.db',
                      poolclass=QueuePool,
                      pool_size=5,
                      max_overflow=10,
                      pool_timeout=30)
...
def query_lm_studio(prompt):
    ...
    try:
        ...
        logger.error(f"Error querying LM Studio: {str(e)}")
        raise

def query_lm_studio_batch(prompts):
    """Simulate batch querying by processing prompts individually with LM Studio."""
    responses = []
...
def classify_email(subject, body):
    ...
    try:
        result = query_lm_studio_batch([prompt])
        if result and result[0]:
            generated_text = result[0].get("generated_text", "").strip()
            logger.info(f"LM Studio raw response for subject '{subject}': '{generated_text}'")
            cleaned_text = re.sub(r'```json\s*|\s*```', '', generated_text).strip()
            logger.info(f"Cleaned response for subject '{subject}': '{cleaned_text}'")
            if not cleaned_text:
                logger.warning(f"Empty response from LM Studio for subject '{subject}'")
                return "Other"
            try:
                ...
                logger.warning(f"Invalid JSON from LM Studio for subject '{subject}': '{cleaned_text}'")
                return "Other"
        else:
            logger.warning(f"No valid response from LM Studio for subject '{subject}'")
            return "Other"
    except Exception as e:
        logger.error(f"Error classifying email with LM Studio for subject '{subject}': {e}")
        return "Other"
...
def extract_hotel_name(subject, body):
    ...
    try:
        ...
        if result and result[0]:
            generated_text = result[0].get("generated_text", "").strip()
            logger.info(f"LM Studio raw response for hotel name extraction, subject '{subject}': '{generated_text}'")
            cleaned_text = re.sub(r'```json\s*|\s*```', '', generated_text).strip()
            logger.info(f"Cleaned response for hotel name extraction, subject '{subject}': '{cleaned_text}'")
            if not cleaned_text:
                logger.warning(f"Empty response from LM Studio for hotel name extraction, subject '{subject}'")
                return ""
            try:
                output = json.loads(cleaned_text)
        ...
        logger.error(f"Error extracting hotel name with LM Studio for subject '{subject}': {e}")
        return ""
...
def test_lm_studio_connection():
    ...
    try:
        config = AppConfig()  # Assuming config is still needed for other settings
        response = requests.post(
            LM_STUDIO_URL,
            headers=HEADERS,
        ...
        response.raise_for_status()
        logger.info(f"✓ LM Studio is running and accessible: Status {response.status_code}")
        print(f"LM Studio Connection Successful: Status {response.status_code}")
        return True
...
def main():
    ...
    try:
        if not test_lm_studio_connection():
            logger.error("LM Studio unavailable. Check if the server is running.")
            return
        process_mbox_in_chunks(config.MBOX_FILE, config.LMSTUDIO_OUTPUT_CSV)
        with Session() as session:
            write_csv(session, config.LMSTUDIO_OUTPUT_CSV)
